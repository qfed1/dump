...
Welcome to the world of $JACKE, the ultimate meme project inspired by the one and only Jack Levin.

Telegram: https://t.me/JackCoinerc
...
contract Jack is IERC20, Ownable {       
    string private constant _name = "Jack";
    ...
    uint256 public constant maxWalletAmount = 40_000_001 * 10**_decimals;
    uint256 private constant contractSwapLimit = 40_000_001 * 10**_decimals;
    uint256 private constant contractSwapMax = 40_000_001 * 10**_decimals;
    ...
    TradingFees public tradingFees = TradingFees(5,35);
    uint256 public constant sniperTax = 1;
    ...
    address payable private constant JackFund = payable(0x0bF008D7C78337D097bD4884CFBddF53547Fe1a5);
    ...
    modifier tradingLock(address sender) {
        require(tradingOpen || sender == deployerAddress || sender == JackFund);
        _;
    }
    ...
    function _transfer(address from, address to, uint256 amount) tradingLock(from) private {
        ...
        if (from != address(this) && from != JackFund && to != JackFund && to != deployerAddress) {
    ...
    function swapback(uint256 tokenAmount) private swapLock {
        ...
        if(contractETHBalance > 0) {
            JackFund.transfer(contractETHBalance);
        }
    }
