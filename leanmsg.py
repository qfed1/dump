...
async def alarm(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    cursor.execute('SELECT eth_address FROM filtered_messages LIMIT 1 OFFSET ?', (timer_beep_counter,))
    row = cursor.fetchone()
    if row is None:
        row = await fetch_message_after_wait(timer_beep_counter)
        if row is None:
            last_refresh_time = 0
            cursor.execute('SELECT eth_address FROM filtered_messages LIMIT 1') 
            row = cursor.fetchone()

    message = f"Row {timer_beep_counter}: {row[0]} - {row[1]}" if row and row[0].strip() and row[1].strip() else "No more messages"
